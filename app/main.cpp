...
int main(int argc, char *argv[])
{
    ...
    auto downloadTile = [&](TileCoord tile) -> VectorTile {
        auto tileLink = tileLoader.downloadTile(Bach::setPbfLink(tile, pbfLinkTemplate.link), networkController);
        ...
        if (tileLink.resultType !=ResultType::success) {
            ...
            auto result = Bach::tileFromByteArray(
                tileLoader.downloadTile(
                    Bach::setPbfLink(tile, pbfLinkTemplate.link), networkController).response);
            if (!result.has_value()) {
                std::abort();
